...
def photo_resizing(sender,instance,*args,**kwargs):
    ...
    output.seek(0)
    instance.image = InMemoryUploadedFile(output,'ImageField', "%s.png" %instance.image.name.split(".")[0], 'image/png', output.len, None)
...
def photo_thumbnail_handler(sender,instance,*args,**kwargs):
    ...
    photo.save(output, format='PNG',optimize=True)
    output.seek(0)
    instance.thumbnail= InMemoryUploadedFile(output,'ImageField', "%s.png" %instance.image.name.split(".")[0], 'image/png', output.len, None)
